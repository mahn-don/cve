...
def search(dte,key):
    ...
    dt={'Affected Product':[],'CVE ID' :[], 'Description' :[], 'CVSSv2' :[], 'CVSSv3' :[], 'Severity' :[], 'Publish Date' :[], 'Last Modified Date' :[], 'cpe23Uri' :[], 'Reference Url' :[]}
    dfr= pd.DataFrame(dt)
    ...
    for i in range(len(df['CVE_Items'])):
        ...
        if x==1:
            ...
            new_row={'Affected Product': productname, 'CVE ID' : CVEid, 'Description' :CVEDescription, 'CVSSv2' :CVEScoreV2, 'CVSSv3' :CVEScoreV3, 'Severity' :CVEserivity, 'Publish Date' :CVEPublishDate, 'Last Modified Date' :CVELastModifiedDate, 'cpe23Uri' :CVECpe23, 'Reference Url' :CVEReferenceUrl}
